...
export function CorrectionCard({ correction, onDeleteClick }: CorrectionCardProps) {
  ...
  return (
    <Card
      className="hover:border-primary/30 hover:shadow-xl transition-all duration-300 group overflow-hidden border-border relative"
    >
      <CardContent className="p-6">
        {/* Delete Button */}
        <button
          onClick={handleToggleConfirm}
          className="absolute top-4 right-4 p-2 rounded-xl bg-muted text-muted-foreground opacity-0 group-hover:opacity-100 hover:bg-red-50 hover:text-red-500 transition-all z-10"
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex flex-col items-center justify-center h-14 w-14 bg-primary/5 rounded-2xl border border-primary/10">
            ...
            <span className="text-[9px] text-muted-foreground uppercase font-bold mt-1">pts</span>
          </div>
          <Badge variant="outline" className="text-[10px] font-black tracking-widest uppercase bg-muted border-border text-muted-foreground">
        ...
        <div className="space-y-4">
          <div>
            <h3 className="font-display font-bold text-foreground text-lg leading-tight group-hover:text-primary transition-colors line-clamp-1 pr-8">
