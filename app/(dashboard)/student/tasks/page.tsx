      ...
      <Card className="group relative overflow-hidden border-none shadow-xl shadow-gray-200/50 bg-card rounded-3xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
        ...
        <CardContent className="p-8">
          <div className="flex items-start justify-between mb-6">
            <div className={cn(
              "p-4 rounded-2xl transition-all duration-500 group-hover:rotate-6 shadow-sm bg-card border border-border",
              config.color
            )}>
            ...
            <div className="flex flex-col items-end gap-2">
              <Badge variant="outline" className={cn(
                ...
                task.status === 'completed' ? "bg-primary/5 text-primary border-primary/10" : "bg-muted text-muted-foreground border-border"
              )}>
                {task.exercise_type}
              </Badge>
              {task.due_date && (
                <span className="text-[10px] text-muted-foreground flex items-center gap-1.5 font-black uppercase tracking-widest bg-muted px-2 py-1 rounded-md">
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-[11px] font-black text-muted-foreground uppercase tracking-widest mt-2 flex items-center gap-1.5">
            ...
            {task.corrections && (
              <div className="p-4 bg-muted rounded-2xl border border-dashed border-border flex items-center justify-between group/basis">
                 <div className="flex items-center gap-2">
                   <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
                   <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Basata sul tuo testo</span>
                 </div>
            ...
            <div className="pt-6 border-t border-border flex items-center justify-between gap-4">
              ...
              <Link href={`/student/tasks/${task.id}`} className="flex-1">
                <Button className={cn(
                  ...
                  task.status === 'completed' ? "bg-muted text-muted-foreground hover:bg-gray-200 shadow-none border-2 border-border/50" : "bg-gray-900 text-white hover:shadow-xl hover:shadow-primary/20 group-hover:bg-primary"
                )}>
    ...
    <div className="max-w-7xl mx-auto space-y-12 py-8 px-4 sm:px-6 lg:px-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-4">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-foreground tracking-tight">Le Tue Attività 🎯</h1>
